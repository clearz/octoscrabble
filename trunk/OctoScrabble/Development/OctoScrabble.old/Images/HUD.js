...
function ControlMenu() {
    ...
    this.pieceClicked = function (event) {
        ...
        if (cMenu.selectedPiece == piece) {
            cMenu.selectedPiece.setPos(cMenu.selectedPiece.posX, cMenu.selectedPiece.posY + cMenu.pieceClickedOffset);
            cMenu.selectedPiece = null;
        }
        else {
            if (cMenu.selectedPiece)
                cMenu.selectedPiece.setPos(cMenu.selectedPiece.posX, cMenu.selectedPiece.posY + cMenu.pieceClickedOffset);
            piece.setPos(piece.posX, piece.posY - cMenu.pieceClickedOffset);
            cMenu.selectedPiece = piece;
        }
    ...
    this.toggleLock = function (event) {
        ...
        var zoom = window.innerWidth / document.documentElement.clientWidth;
        cMenu.Lock.width = (ScreenLocked ? cMenu.IconSize : cMenu.OpenLockWidth) * zoom * cMenu.mult;
        cMenu.Lock.src = ScreenLocked ? "images/lockc.png" : "images/locko.png";
		if(event)
		{
			event.cancelBubble = true;
			event.preventDefault();
    ...
    this.Show = function () {
        if (window.pageXOffset != cMenu.lastX) {
            cMenu.lastX = window.pageXOffset;
            setTimeout(cMenu.Show, 20);
        }
        else {
            cMenu.lastX = -1;
            cMenu.IShow();
    ...
    this.fadeIn = function () {
        if (cMenu.opacity < cMenu.targetOpacity) {
            cMenu.opacity += 0.01;
            cMenu.Element.style.opacity = cMenu.opacity;
			cMenu.TopElement.style.opacity = cMenu.opacity;
            setTimeout(cMenu.fadeIn, 4);
        }
    }
    this.IShow = function () {
		...
		this.Lock.width = ((ScreenLocked ? cMenu.IconSize : cMenu.OpenLockWidth) * zoom * this.mult);
 ...
 this.Lock.width = (ScreenLocked ? cMenu.IconSize : cMenu.OpenLockWidth) * zoom;
 this.Lock.src = ScreenLocked ? "images/lockc.png" : "images/locko.png";
