...
-(void) playSound:(NSString*)filename{
	//if(!soundOn)return;
	if([g_ResManager userDataExists:@"playSound.on"] && [[g_ResManager getUserData:@"playSound.on"] boolValue])
	{
		if(DEBUG_SOUND_ENABLED)
...
-(void) playMusic:(NSString*)filename{	
	if([g_ResManager userDataExists:@"playMusic.on"] && [[g_ResManager getUserData:@"playMusic.on"] boolValue])
	{
		NSString *fullpath = [[[NSBundle mainBundle] bundlePath] stringByAppendingPathComponent:filename];
